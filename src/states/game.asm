...
;src/states/game.c:146: DISPLAY_OFF;
	call	_display_off
;src/states/game.c:147: set_bkg_data(0, 40, tiles_data); // All tiles (now 40)
	ld	de, #_tiles_data
	push	de
	ld	hl, #0x2800
	push	hl
...
;src/states/game.c:167: set_sprite_data(40, 1, projectile_sprite);
	...
	call	_set_sprite_data
	add	sp, #4
;src/states/game.c:168: set_sprite_data(41, 2, flower_sprite);
	...
	call	_set_sprite_data
	add	sp, #4
;src/states/game.c:169: update_player_sprite();
	call	_update_player_sprite
;src/states/game.c:170: text_init();
	call	_text_init
;src/states/game.c:171: music_init();
	call	_music_init
;src/states/game.c:172: projectile_init();
	call	_projectile_init
;src/states/game.c:173: SHOW_BKG;
	ldh	a, (_LCDC_REG + 0)
	or	a, #0x01
	ldh	(_LCDC_REG + 0), a
;src/states/game.c:174: SHOW_SPRITES;
	ldh	a, (_LCDC_REG + 0)
	or	a, #0x02
	ldh	(_LCDC_REG + 0), a
;src/states/game.c:175: DISPLAY_ON;
	ldh	a, (_LCDC_REG + 0)
	or	a, #0x80
	ldh	(_LCDC_REG + 0), a
;src/states/game.c:176: game_ready = 1;
	ld	hl, #_game_ready
	ld	(hl), #0x01
;src/states/game.c:177: }
	ret
;src/states/game.c:179: void game_update(void) {
;	---------------------------------
; Function game_update
; ---------------------------------
_game_update::
	add	sp, #-12
;src/states/game.c:180: music_update();
	call	_music_update
;src/states/game.c:181: uint8_t moved = 0;
	ld	c, #0x00
;src/states/game.c:182: uint16_t nx = player_x, ny = player_y;
	ld	a, (#_player_x)
	ldhl	sp,	#8
	ld	(hl), a
	ld	a, (#_player_x + 1)
	...
	ldhl	sp,	#11
	ld	(hl), a
;src/states/game.c:184: if (input_held(J_UP)) {
	push	bc
	ld	a, #0x04
	call	_input_held
	pop	bc
...
;src/states/game.c:185: ny--;
	...
	ld	(hl+), a
	ld	(hl), d
;src/states/game.c:186: player_dir = 1;
	ld	hl, #_player_dir
	ld	(hl), #0x01
;src/states/game.c:187: moved = 1;
	ld	c, #0x01
	jr	00111$
00110$:
;src/states/game.c:188: } else if (input_held(J_DOWN)) {
	push	bc
	ld	a, #0x08
	call	_input_held
	pop	bc
...
;src/states/game.c:189: ny++;
	...
	inc	(hl)
00670$:
;src/states/game.c:190: player_dir = 0;
	xor	a, a
	ld	(#_player_dir),a
;src/states/game.c:191: moved = 1;
	ld	c, #0x01
	jr	00111$
00107$:
;src/states/game.c:192: } else if (input_held(J_LEFT)) {
	push	bc
	ld	a, #0x02
	call	_input_held
	pop	bc
...
;src/states/game.c:193: nx--;
	...
	ld	(hl+), a
	ld	(hl), d
;src/states/game.c:194: player_dir = 2;
	ld	hl, #_player_dir
	ld	(hl), #0x02
;src/states/game.c:195: moved = 1;
	ld	c, #0x01
	jr	00111$
00104$:
;src/states/game.c:196: } else if (input_held(J_RIGHT)) {
	push	bc
	ld	a, #0x01
	call	_input_held
	pop	bc
...
;src/states/game.c:197: nx++;
	...
	inc	(hl)
00671$:
;src/states/game.c:198: player_dir = 3;
	ld	hl, #_player_dir
	ld	(hl), #0x03
;src/states/game.c:199: moved = 1;
	ld	c, #0x01
00111$:
;src/states/game.c:202: if (moved) {
	ld	a, c
	or	a, a
	jr	Z, 00135$
;src/states/game.c:203: if (can_move(nx, ny)) {
	ldhl	sp,	#10
	ld	a, (hl+)
	ld	c, a
	ld	b, (hl)
	...
	or	a, a
	jp	Z, 00136$
;src/states/game.c:204: player_x = nx;
	ldhl	sp,	#8
	ld	a, (hl)
	ld	(#_player_x),a
	ldhl	sp,	#9
	ld	a, (hl)
	ld	(#_player_x + 1),a
;src/states/game.c:205: player_y = ny;
	ldhl	sp,	#10
	ld	a, (hl)
	ld	(#_player_y),a
	ldhl	sp,	#11
...
;src/states/game.c:206: update_camera();
	call	_update_camera
;src/states/game.c:208: if (++anim_timer > 6) {
	ld	hl, #_anim_timer
	inc	(hl)
	ld	a, #0x06
	sub	a, (hl)
	jp	NC, 00136$
;src/states/game.c:209: anim_frame = !anim_frame;
	ld	hl, #_anim_frame
	ld	a, (hl)
	sub	a, #0x01
	ld	a, #0x00
	rla
	ld	(hl), a
;src/states/game.c:210: anim_timer = 0;
	xor	a, a
	ld	(#_anim_timer),a
;src/states/game.c:211: update_player_sprite();
	call	_update_player_sprite
	jp	00136$
00135$:
;src/states/game.c:215: if (anim_frame != 0) {
	ld	hl, #_anim_frame
	ld	a, (hl)
	or	a, a
	jr	Z, 00117$
;src/states/game.c:216: anim_frame = 0;
	ld	(hl), #0x00
;src/states/game.c:217: update_player_sprite();
	call	_update_player_sprite
00117$:
;src/states/game.c:219: anim_timer = 0;
	xor	a, a
	ld	(#_anim_timer),a
;src/states/game.c:222: if (input_pressed(J_SELECT)) {
	ld	a, #0x40
	call	_input_pressed
	or	a, a
	jr	Z, 00119$
;src/states/game.c:223: game_state = 3; // STATE_INVENTORY
	ld	hl, #_game_state
	ld	(hl), #0x03
;src/states/game.c:224: return;
	jp	00218$
00119$:
;src/states/game.c:228: uint8_t tid = get_tile_at(player_x + 8, player_y + 4);
	ld	a, (_player_y)
	ld	c, a
	ld	hl, #_player_y + 1
	ld	b, (hl)
...
00672$:
	...
	ldhl	sp,	#11
	ld	(hl), a
;src/states/game.c:229: if (current_map == &maps[0] && // WORLD_MAP
;src/states/game.c:228: uint8_t tid = get_tile_at(player_x + 8, player_y + 4);
	ld	a, (_player_y)
	ld	c, a
	ld	hl, #_player_y + 1
	...
	ld	hl, #_player_x + 1
	ld	d, (hl)
;src/states/game.c:234: switch_map(&maps[1], 76, 112); // HOUSE_MAP
;src/states/game.c:229: if (current_map == &maps[0] && // WORLD_MAP
	ld	hl, #_current_map
	ld	a, (hl)
	sub	a, #<(_maps)
	...
	sub	a, #>(_maps)
	jr	NZ, 00124$
;src/states/game.c:230: (tid == 21 || tid == 22)) {
	ldhl	sp,	#11
	ld	a, (hl)
	sub	a, #0x15
	jr	Z, 00123$
	...
	jr	NZ, 00124$
00123$:
;src/states/game.c:231: if (player_x > 160 && player_y > 160) {
	ld	a, #0xa0
	cp	a, e
	ld	a, #0x00
	sbc	a, d
...
;src/states/game.c:234: switch_map(&maps[1], 76, 112); // HOUSE_MAP
	...
	ld	de, #(_maps + 26)
	call	_switch_map
;src/states/game.c:235: return;
	jp	00218$
00124$:
;src/states/game.c:238: tid = get_tile_at(player_x + 8, player_y + 12);
	ld	hl, #0x000c
	add	hl, bc
	ld	c, l
	ld	b, h
	...
	call	_get_tile_at
	ld	c, a
;src/states/game.c:239: if (current_map == &maps[1] && tid == 35) { // HOUSE_MAP
	ld	hl, #_current_map
	ld	a, (hl)
	sub	a, #<((_maps + 26))
	jr	NZ, 00128$
	...
	sub	a, #0x23
	jr	NZ, 00128$
;src/states/game.c:240: uint16_t wx = saved_world_x;
	ld	a, (_saved_world_x)
	ld	hl, #_saved_world_x + 1
	ld	b, (hl)
;src/states/game.c:241: uint16_t wy = saved_world_y;
	ld	hl, #_saved_world_y
	ld	e, (hl)
	inc	hl
	ld	d, (hl)
;src/states/game.c:242: switch_map(&maps[0], wx, wy); // WORLD_MAP
	push	de
	ld	c, a
	ld	de, #_maps
	call	_switch_map
;src/states/game.c:243: return;
	jp	00218$
00128$:
;src/states/game.c:245: if (current_map == &maps[2] && tid == 0 && player_y > 240) { // LEVEL2_MAP
	ld	hl, #_current_map
	ld	a, (hl)
	sub	a, #<((_maps + 52))
	jr	NZ, 00136$
	...
	sbc	a, b
	jr	NC, 00136$
;src/states/game.c:246: switch_map(&maps[0], 124, 48);                             // WORLD_MAP
	ld	de, #0x0030
	push	de
	ld	bc, #0x007c
	ld	de, #_maps
	call	_switch_map
;src/states/game.c:247: return;
	jp	00218$
00136$:
...
;src/states/game.c:254: for (int i = 0; i < 4; i++)
	...
	jr	00207$
00137$:
;src/states/game.c:258: entity_update_all(current_map->entities, current_map->num_entities);
	ld	a, (_current_map)
	ld	e, a
	ld	hl, #_current_map + 1
	ld	d, (hl)
...
;src/states/game.c:261: if (current_map == &maps[2]) { // LEVEL2_MAP
	ld	hl, #_current_map
	ld	a, (hl)
	sub	a, #<((_maps + 52))
	jp	NZ, 00143$
	...
	sub	a, #>((_maps + 52))
	jr	NZ, 00143$
;src/states/game.c:262: for (int i = 0; i < current_map->num_entities; i++) {
	xor	a, a
	ldhl	sp,	#10
	ld	(hl+), a
	ld	(hl), a
...
00687$:
	jr	NC, 00143$
;src/states/game.c:263: entity_t *e = &current_map->entities[i];
	ld	hl, #0x0004
	add	hl, bc
	ld	a, (hl+)
	ld	c, a
	...
	ld	e, l
	ld	d, h
;src/states/game.c:264: if (e->active && e->type == ENT_ENEMY) {
	ld	hl, #0x0008
	add	hl, de
	ld	a, (hl)
	or	a, a
	...
	sub	a, #0x03
	jr	NZ, 00211$
;src/states/game.c:265: ai_enemy_shooter(e, player_x, player_y);
	ld	a, (_player_y)
	ld	c, a
	ld	hl, #_player_y + 1
	ld	b, (hl)
...
;src/states/game.c:262: for (int i = 0; i < current_map->num_entities; i++) {
	...
	jr	00210$
00143$:
;src/states/game.c:271: projectile_update_all();
	call	_projectile_update_all
;src/states/game.c:272: projectile_render_all(camera_x, camera_y);
	ld	a, (_camera_y)
	ld	c, a
	ld	hl, #_camera_y + 1
	ld	b, (hl)
...
;src/states/game.c:275: if (projectile_check_collision(player_x + 8, player_y + 8)) {
	ld	a, (_player_y)
	ld	hl, #_player_y + 1
	ld	b, (hl)
	add	a, #0x08
...
00692$:
	...
	or	a, a
	jr	Z, 00261$
;src/states/game.c:277: fade_out();
	call	_fade_out
;src/states/game.c:278: delay(500);
	ld	de, #0x01f4
	call	_delay
;src/states/game.c:279: player_x = 128;
	ld	hl, #_player_x
	ld	a, #0x80
	ld	(hl+), a
	xor	a, a
	ld	(hl), a
;src/states/game.c:280: player_y = 224;
	ld	hl, #_player_y
	ld	a, #0xe0
	ld	(hl+), a
	xor	a, a
	ld	(hl), a
;src/states/game.c:281: load_map(&maps[2]); // LEVEL2_MAP
	ld	de, #(_maps + 52)
	call	_load_map
;src/states/game.c:282: update_player_sprite();
	call	_update_player_sprite
;src/states/game.c:283: fade_in();
	call	_fade_in
;src/states/game.c:284: return;
	jp	00218$
;src/states/game.c:288: for (int i = 0; i < current_map->num_entities; i++) {
00261$:
	xor	a, a
	ldhl	sp,	#6
	ld	(hl+), a
...
;src/states/game.c:258: entity_update_all(current_map->entities, current_map->num_entities);
	ld	hl, #0x0004
	add	hl, bc
;src/states/game.c:289: entity_t *e = &current_map->entities[i];
	ld	a, (hl+)
	ld	c, a
	ld	b, (hl)
;src/states/game.c:288: for (int i = 0; i < current_map->num_entities; i++) {
...
;src/states/game.c:289: entity_t *e = &current_map->entities[i];
	...
	ld	c, l
	ld	b, h
;src/states/game.c:290: if (e->active && e->type == ENT_ENEMY) {
	ld	hl, #0x0008
	add	hl, bc
	ld	a, (hl)
	or	a, a
	...
	sub	a, #0x03
	jp	NZ, 00214$
;src/states/game.c:291: int16_t dx = (int16_t)player_x - (int16_t)e->x;
	ld	a, (#_player_x)
	ldhl	sp,	#4
	ld	(hl), a
	ld	a, (#_player_x + 1)
	...
	ld	(hl-), a
	ld	(hl), e
;src/states/game.c:292: int16_t dy = (int16_t)player_y - (int16_t)e->y;
	ld	a, (#_player_y)
	ldhl	sp,	#4
	ld	(hl), a
	ld	a, (#_player_y + 1)
...
;src/states/game.c:294: dx = -dx;
	...
	ld	(hl), e
00147$:
;src/states/game.c:295: if (dy < 0)
	ldhl	sp,	#10
	ld	a, (hl+)
	bit	7, (hl)
	jr	Z, 00149$
;src/states/game.c:296: dy = -dy;
	ld	de, #0x0000
	ld	a, (hl-)
	ld	l, (hl)
	ld	h, a
	...
	ld	(hl), e
00149$:
;src/states/game.c:297: if (dx < 10 && dy < 10) {
	ldhl	sp,	#8
	ld	a, (hl+)
	ld	c, a
	ld	b, (hl)
	...
	sbc	a, #0x80
	jr	NC, 00214$
;src/states/game.c:298: fade_out();
	call	_fade_out
;src/states/game.c:299: delay(1000);
	ld	de, #0x03e8
	call	_delay
;src/states/game.c:300: player_x = 128;
	ld	hl, #_player_x
	ld	a, #0x80
	ld	(hl+), a
	xor	a, a
	ld	(hl), a
;src/states/game.c:301: player_y = 224;
	ld	hl, #_player_y
	ld	a, #0xe0
	ld	(hl+), a
	xor	a, a
	ld	(hl), a
;src/states/game.c:302: load_map(&maps[2]); // LEVEL2_MAP
	ld	de, #(_maps + 52)
	call	_load_map
;src/states/game.c:303: update_player_sprite();
	call	_update_player_sprite
;src/states/game.c:304: fade_in();
	call	_fade_in
;src/states/game.c:305: return;
	jp	00218$
00214$:
;src/states/game.c:288: for (int i = 0; i < current_map->num_entities; i++) {
	ldhl	sp,	#6
	...
	jp	00213$
00156$:
;src/states/game.c:310: entity_render_all(current_map->entities, current_map->num_entities, camera_x,
	ld	a, #0x04
	push	af
	inc	sp
	ld	a, (_camera_y)
...
;src/states/game.c:313: if (input_pressed(J_A | J_B)) {
	ld	a, #0x30
	call	_input_pressed
	or	a, a
	jp	Z, 00202$
;src/states/game.c:314: for (int i = 0; i < current_map->num_entities; i++) {
	xor	a, a
	ldhl	sp,	#6
	ld	(hl+), a
	ld	(hl), a
...
00699$:
	jp	NC, 00180$
;src/states/game.c:315: entity_t *e = &current_map->entities[i];
	ldhl	sp,#8
	ld	a, (hl+)
	ld	e, a
	ld	d, (hl)
	...
	inc	sp
	push	hl
;src/states/game.c:316: int16_t dx = (int16_t)player_x - (int16_t)e->x,
	ld	a, (#_player_x)
	ldhl	sp,	#4
	ld	(hl), a
	ld	a, (#_player_x + 1)
	...
	ld	(hl-), a
	ld	(hl), e
;src/states/game.c:317: dy = (int16_t)player_y - (int16_t)e->y;
	ld	a, (#_player_y)
	ldhl	sp,	#2
	ld	(hl), a
	ld	a, (#_player_y + 1)
...
;src/states/game.c:319: dx = -dx;
	...
	ld	(hl), e
00158$:
;src/states/game.c:320: if (dy < 0)
	ldhl	sp,	#10
	ld	a, (hl+)
	bit	7, (hl)
	jr	Z, 00160$
;src/states/game.c:321: dy = -dy;
	ld	de, #0x0000
	ld	a, (hl-)
	ld	l, (hl)
	ld	h, a
	...
	ld	(hl), e
00160$:
;src/states/game.c:322: if (dx < 24 && dy < 24) {
	ldhl	sp,	#8
	ld	a, (hl+)
	ld	c, a
	ld	b, (hl)
	...
	sbc	a, #0x80
	jp	NC, 00217$
;src/states/game.c:323: if (input_pressed(J_A)) {
	ld	a, #0x10
	call	_input_pressed
	ldhl	sp,	#11
	ld	(hl), a
;src/states/game.c:324: if (e->type == ENT_NPC) {
	pop	de
	push	de
	ld	hl, #0x0004
	add	hl, de
	...
	ldhl	sp,	#9
	ld	(hl), a
;src/states/game.c:326: if (e->sprite_base == 36 && inventory_has_item("FLOR")) {
	pop	de
	push	de
	ld	hl, #0x0007
	add	hl, de
...
;src/states/game.c:326: if (e->sprite_base == 36 && inventory_has_item("FLOR")) {
	...
	or	a, a
	jr	Z, 00162$
;src/states/game.c:327: text_dialogue(DIALOGUE_FLOWER_THANKS);
	push	bc
	ld	de, #___str_1
	call	_text_dialogue
	pop	bc
	jr	00169$
00162$:
;src/states/game.c:329: text_dialogue(e->dialogue);
	pop	de
	push	de
	ld	hl, #0x0005
	add	hl, de
	...
	call	_text_dialogue
	pop	bc
;src/states/game.c:331: } else if (e->type == ENT_ITEM) {
00169$:
;src/states/game.c:335: if (input_pressed(J_B)) {
	push	bc
	ld	a, #0x20
	call	_input_pressed
	pop	bc
	or	a, a
	jr	Z, 00217$
;src/states/game.c:336: if (e->type == ENT_ITEM && e->active) {
	ldhl	sp,#8
	ld	a, (hl+)
	ld	e, a
	ld	d, (hl)
	...
	or	a, a
	jr	Z, 00217$
;src/states/game.c:339: if (e->sprite_base == 41) {
	ld	a, (bc)
	sub	a, #0x29
	jr	NZ, 00217$
;src/states/game.c:340: inventory_add_item("FLOR", "UNA HERMOSA\nFLOR SILVESTRE", 41);
	push	de
	ld	a, #0x29
	push	af
	inc	sp
...
;src/states/game.c:341: sfx_pickup();
	call	_sfx_pickup
	pop	de
;src/states/game.c:342: e->active = 0; // Remove from map
	xor	a, a
	ld	(de), a
00217$:
;src/states/game.c:314: for (int i = 0; i < current_map->num_entities; i++) {
	...
	jp	00216$
00180$:
;src/states/game.c:350: uint8_t tid = get_tile_at(player_x + 8, player_y);
	ld	a, (_player_x)
	ld	hl, #_player_x + 1
	ld	d, (hl)
	add	a, #0x08
...
00707$:
	...
	ldhl	sp,	#10
	ld	(hl), a
;src/states/game.c:351: if (player_dir == 1)
	ld	a, (#_player_dir)
	dec	a
	jr	NZ, 00182$
;src/states/game.c:352: tid = get_tile_at(player_x + 8, player_y - 4);
	ld	a, (_player_y)
	ld	hl, #_player_y + 1
	ld	b, (hl)
	add	a, #0xfc
...
00710$:
	...
	ld	(hl), a
00182$:
;src/states/game.c:354: if (current_map == &maps[1] && // HOUSE_MAP
	ld	hl, #_current_map
	ld	a, (hl)
	sub	a, #<((_maps + 26))
	jr	NZ, 00189$
	...
	sub	a, #>((_maps + 26))
	jr	NZ, 00189$
;src/states/game.c:355: (tid == 31 || tid == 32 || tid == 33 || tid == 34)) {
	ldhl	sp,	#10
	ld	a, (hl)
	sub	a, #0x1f
	jr	Z, 00188$
	...
	jr	NZ, 00189$
00188$:
;src/states/game.c:356: if (input_pressed(J_B)) {
	ld	a, #0x20
	call	_input_pressed
	ldhl	sp,#11
	ld	(hl), a
	...
	or	a, a
	jr	Z, 00189$
;src/states/game.c:357: if (!inventory_has_item("LLAVE")) {
	ld	de, #___str_3
	call	_inventory_has_item
	ldhl	sp,#11
	ld	(hl), a
	...
	or	a, a
	jr	NZ, 00184$
;src/states/game.c:358: text_dialogue(DIALOGUE_FOUND_KEY);
	ld	de, #___str_4
	call	_text_dialogue
;src/states/game.c:359: inventory_add_item("LLAVE", "ABRE EL PORTON NORTE", 41);
	ld	a, #0x29
	push	af
	inc	sp
	ld	bc, #___str_5
	ld	de, #___str_3
	call	_inventory_add_item
;src/states/game.c:360: sfx_pickup();
	call	_sfx_pickup
	jr	00189$
00184$:
;src/states/game.c:362: text_dialogue(DIALOGUE_EMPTY_CHEST);
	ld	de, #___str_6
	call	_text_dialogue
00189$:
;src/states/game.c:366: if (current_map == &maps[0] && (tid == 38 || tid == 39)) { // WORLD_MAP
	ld	hl, #_current_map
	ld	a, (hl)
	sub	a, #<(_maps)
	jr	NZ, 00202$
	...
	jr	NZ, 00202$
00197$:
;src/states/game.c:367: if (inventory_has_item("LLAVE")) {
	ld	de, #___str_3
	call	_inventory_has_item
	or	a, a
	jr	Z, 00195$
;src/states/game.c:368: text_dialogue(DIALOGUE_USE_KEY);
	ld	de, #___str_7
	call	_text_dialogue
;src/states/game.c:369: sfx_success();
	call	_sfx_success
;src/states/game.c:370: switch_map(&maps[2], 128, 224); // LEVEL2_MAP
	ld	de, #0x00e0
	push	de
	ld	bc, #0x0080
	ld	de, #(_maps + 52)
...
;src/states/game.c:371: return;
	jr	00218$
00195$:
;src/states/game.c:373: text_dialogue(DIALOGUE_LOCKED_GATE);
	ld	de, #___str_8
	call	_text_dialogue
00202$:
;src/states/game.c:378: if ((env_anim_timer % 32) == 0) {
	ld	a, (_env_anim_timer)
	and	a, #0x1f
	jr	NZ, 00204$
;src/states/game.c:379: env_anim_frame = !env_anim_frame;
	...
	rla
	ld	(hl), a
;src/states/game.c:383: set_bkg_data(2, 1, env_anim_frame ? &tiles_anim_data[0] : &tiles_data[32]);
	ld	a, (hl)
	or	a, a
	jr	Z, 00224$
...
00225$:
	...
	add	sp, #4
00204$:
;src/states/game.c:385: env_anim_timer++;
	ld	hl, #_env_anim_timer
	inc	(hl)
00218$:
;src/states/game.c:386: }
	add	sp, #12
	ret
___str_0:
	.ascii "FLOR"
