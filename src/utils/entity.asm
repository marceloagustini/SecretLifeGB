...
;src/utils/entity.c:68: }
	add	sp, #4
	ret
;src/utils/entity.c:70: void entity_update_all(entity_t *entities, uint8_t count) {
;	---------------------------------
...
_entity_update_all::
	...
	dec	hl
	ld	(hl), a
;src/utils/entity.c:71: for (uint8_t i = 0; i < count; i++) {
	ldhl	sp,	#7
	ld	(hl), #0x00
00106$:
	ldhl	sp,	#7
	...
	sub	a, (hl)
	jr	NC, 00108$
;src/utils/entity.c:72: if (entities[i].active && entities[i].update) {
	ldhl	sp,	#7
	ld	c, (hl)
	ld	b, #0x00
	ld	l, c
	...
	or	a, c
	jr	Z, 00107$
;src/utils/entity.c:73: entities[i].update(&entities[i]);
	ldhl	sp,	#2
	ld	a, (hl+)
	ld	e, a
	ld	d, (hl)
...
;src/utils/entity.c:76: }
	add	sp, #8
	ret
;src/utils/entity.c:78: void entity_render_all(entity_t *entities, uint8_t count, uint16_t camera_x,
;	---------------------------------
...
_entity_render_all::
	...
	dec	hl
	ld	(hl), a
;src/utils/entity.c:80: for (uint8_t i = 0; i < count; i++) {
	ldhl	sp,	#12
	ld	(hl), #0x00
00126$:
	ldhl	sp,	#12
	...
	sub	a, (hl)
	jp	NC, 00127$
;src/utils/entity.c:81: entity_t *e = &entities[i];
	ldhl	sp,	#12
	ld	c, (hl)
	ld	b, #0x00
	ld	l, c
	...
	ld	a, h
	ldhl	sp,	#8
;src/utils/entity.c:82: if (!e->active) {
	ld	(hl-), a
	ld	a, (hl+)
	ld	e, a
	ld	d, (hl)
	...
	ld	b, h
	ld	a, (bc)
;src/utils/entity.c:83: for (int j = 0; j < 4; j++)
	or	a, a
	jr	NZ, 00103$
	ld	c, a
00117$:
	...
	sub	a, #0x04
	jp	NC, 00110$
;src/utils/entity.c:84: move_sprite(sprite_offset + (i * 4) + j, 0, 0);
	ldhl	sp,	#12
	ld	a, (hl)
	add	a, a
	add	a, a
...
;src/utils/entity.c:83: for (int j = 0; j < 4; j++)
	inc	c
	jr	00117$
;src/utils/entity.c:85: continue;
00103$:
;src/utils/entity.c:88: uint16_t esx = e->x - camera_x + 8, esy = e->y - camera_y + 16;
	ldhl	sp,#7
	ld	a, (hl+)
	ld	e, a
	ld	d, (hl)
	...
	inc	hl
	ld	(hl), a
;src/utils/entity.c:89: if (esx < 168 && esy < 160) {
	pop	bc
	push	bc
	ld	a, c
	sub	a, #0xa8
	...
	sbc	a, #0x00
	jp	NC, 00144$
;src/utils/entity.c:90: uint8_t frame_offset = (e->anim_frame * 4);
	ldhl	sp,#7
	ld	a, (hl+)
	ld	e, a
	ld	d, (hl)
	...
	ldhl	sp,	#4
	ld	(hl), a
;src/utils/entity.c:91: for (int j = 0; j < 4; j++) {
	ldhl	sp,#7
	ld	a, (hl+)
	ld	e, a
	ld	d, (hl)
...
00120$:
	...
	sub	a, #0x04
	jp	NC, 00110$
;src/utils/entity.c:92: uint8_t sprite_id = sprite_offset + (i * 4) + j;
	dec	hl
	ld	a, (hl)
	add	a, a
	add	a, a
	...
	ldhl	sp,	#7
	ld	(hl), a
;src/utils/entity.c:93: move_sprite(sprite_id, esx + (j % 2 ? 8 : 0), esy + (j >= 2 ? 8 : 0));
	ldhl	sp,	#2
	ld	a, (hl)
	ldhl	sp,	#8
	ld	(hl), a
...
;./gbdk/include/gb/gb.h:1974: itm->y=y, itm->x=x;
	...
	ld	(hl+), a
	ld	(hl), b
;src/utils/entity.c:94: set_sprite_tile(sprite_id, e->sprite_base + frame_offset + j);
	ldhl	sp,#5
	ld	a, (hl+)
	ld	e, a
	ld	a, (hl-)
...
;src/utils/entity.c:91: for (int j = 0; j < 4; j++) {
	...
	inc	(hl)
	jr	00120$
;src/utils/entity.c:97: for (int j = 0; j < 4; j++)
00144$:
	ld	c, #0x00
00123$:
	ld	a, c
	sub	a, #0x04
	jr	NC, 00110$
;src/utils/entity.c:98: move_sprite(sprite_offset + (i * 4) + j, 0, 0);
	ldhl	sp,	#12
	ld	a, (hl)
	add	a, a
	add	a, a
